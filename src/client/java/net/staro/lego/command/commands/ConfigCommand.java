...
public class ConfigCommand extends LegoCommand {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.executes(context -> {
            ...
            return COMPLETED;
        }).then(literal("save").then(argument("name", StringArgument.string()).executes(context -> {
        ...
        ).then(literal("load").then(argument("config", configArgument).executes(context -> {
        ...
        ).then(literal("del").then(argument("config", configArgument).executes(context -> {
            String config = context.getArgument("config", String.class);
